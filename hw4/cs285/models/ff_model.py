...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = normalize(obs_unnormalized, obs_mean, obs_std)  # (Q1)
        acs_normalized = normalize(acs_unnormalized, acs_mean, acs_std)  # (Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        # prediction = # (Q1) get the predicted next-states (s_t+1) as a numpy array
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        # target = # (Q1) compute the normalized target for the model.
        ...
        target_delta = normalize(
            ...
            data_stats_tensor["delta_std"],
        )
        _, predicted_delta = self.forward(observations, actions, **data_stats_tensor)

        loss = self.loss_func(predicted_delta, target_delta)  # (Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
